...
class DataGenerator(Dataset):
    def __init__(self, Cfg, back_list=[0], fore_list=[0], order_list=[0], phase="train", test_scale="resize", crop_size = 320, augmentation=True):
        global cfg
        cfg = Cfg
        self.phase = phase
        self.crop_size = cfg.TRAIN.crop_size
        self.back_list = back_list
        self.fore_list = fore_list
        self.order_list = order_list
        self.sample_per_tuple = 4
        self.augmentation = augmentation
        self.composition_style = cfg.DATASET.composition_style
        if self.phase == "train":
    ...
    def __getitem__(self, idx):
        if self.phase == "train":
            if self.composition_style is not None:
                idx = self.order_list[idx]
                tuple_index = idx // self.sample_per_tuple
                sample_tuple_index = idx % self.sample_per_tuple

                foreground0 = self.fore_list[tuple_index][0]
                foreground1 = self.fore_list[tuple_index][1]
                background0 = self.back_list[tuple_index][0]
                background1 = self.back_list[tuple_index][1]


                if not cfg.TRAIN.load_data:
                        fg0 = cv2.imread(self.fg[foreground0 % self.fg_num])
                        fg1 = cv2.imread(self.fg[foreground1 % self.fg_num])
                        alpha0 = cv2.imread(self.alpha[foreground0 % self.fg_num], 0).astype(np.float32) / 255
                        alpha1 = cv2.imread(self.alpha[foreground1 % self.fg_num], 0).astype(np.float32) / 255
                else:
                    fg0_name = self.fg[foreground0 % self.fg_num]
                    fg0_name = fg0_name[fg0_name.rfind('/')+1:-4]
                    fg1_name = self.fg[foreground1 % self.fg_num]
                    fg1_name = fg1_name[fg1_name.rfind('/')+1:-4]

                    fg0 = self.fg_load[fg0_name]
                    fg1 = self.fg_load[fg1_name]
                    alpha0 = self.alpha_load[fg0_name] 
                    alpha1 = self.alpha_load[fg1_name] 
                if not cfg.TRAIN.load_bg:
                        bg0 = cv2.imread(self.bg[background0 % self.bg_num], 1)
                        bg1 = cv2.imread(self.bg[background1 % self.bg_num], 1)
                else:
                        bg0_name = self.bg[background0 % self.bg_num]
                        bg0_name = bg0_name[bg0_name.rfind('/') + 1:-4]
                        bg0 = self.bg_load[bg0_name]                        
                        bg1_name = self.bg[background1 % self.bg_num]
                        bg1_name = bg1_name[bg1_name.rfind('/') + 1:-4]
                        bg1 = self.bg_load[bg1_name]
                if bg0.shape[2]==1:
                    bg0 = np.repeat(bg0, 3, axis=2)
                if bg1.shape[2]==1:
                    bg1 = np.repeat(bg1, 3, axis=2)
                alpha0 = np.squeeze(alpha0)
                alpha1 = np.squeeze(alpha1)

                
                if self.composition_style=='quad':      #quad

                    
                    if sample_tuple_index == 0:
                        sample = {'fg': fg0, 'alpha': alpha0, 'bg': bg0, 'image_name': 'm'}
                    elif sample_tuple_index == 1:
                        sample = {'fg': fg1, 'alpha': alpha1, 'bg': bg1, 'image_name': 'm'}
                    elif sample_tuple_index == 2:
                        sample = composite_alpha_resize_one(fg0, alpha0, fg1, alpha1, bg0)
                    elif sample_tuple_index == 3:
                        sample = composite_alpha_resize_one(fg1, alpha1, fg0, alpha0, bg1)
                        

                elif self.composition_style=='tri':  #tri
                    if sample_tuple_index == 0:
                        sample = {'fg': fg0, 'alpha': alpha0, 'bg': bg0, 'image_name': 'm'}
                    elif sample_tuple_index == 1:
                        sample = {'fg': fg1, 'alpha': alpha1, 'bg': bg1, 'image_name': 'm'}
                ...
                """
                if you need to ensure the combined foregrounds appear with the probability of 0.5
                elif self.composition_style=='tri':  #tri
                    if sample_tuple_index == 0:
                        sample = {'fg': fg0, 'alpha': alpha0, 'bg': bg0, 'image_name': 'm'}
                    elif sample_tuple_index == 1:
                        sample = {'fg': fg1, 'alpha': alpha1, 'bg': bg1, 'image_name': 'm'}
                    elif sample_tuple_index == 2 or sample_tuple_index == 3:
                        sample = composite_alpha_resize_one(fg0, alpha0, fg1, alpha1, bg0)
                sample = self.transform(sample)
                return sample
                """
            ...
            image = cv2.imread(os.path.join(cfg.DATASET.test_dir, self.test_list[idx][0]))
            alpha = cv2.imread(os.path.join(cfg.DATASET.test_dir, self.test_list[idx][1]), 0).astype(np.float32) / 255.
            alpha = alpha[:, :, 0] if alpha.ndim == 3 else alpha
            trimap = cv2.imread(os.path.join(cfg.DATASET.test_dir, self.test_list[idx][2]), 0)
    ...
    def __len__(self):
        if self.phase == "train":
            return cfg.TRAIN.batch_size*cfg.TRAIN.epoch_iterations
        else:
            return len(self.test_list)

def composite_alpha_resize_one(fg1, alpha1, fg2, alpha2, bg):
    ##先resize到一个大小，再叠加到上方，trimap产生方式是将其作为前景，再dilation
    alpha1 = np.squeeze(alpha1)
    alpha2 = np.squeeze(alpha2)
    h, w = alpha1.shape
    fg2 = cv2.resize(fg2, (w, h), interpolation=cv2.INTER_NEAREST)
    alpha2 = cv2.resize(alpha2, (w, h), interpolation=cv2.INTER_NEAREST)
    alpha_new = 1 - (1-alpha1) * (1-alpha2)
    fg_new = ((alpha1[:,:,None] * fg1 + ((1 - alpha1[:,:,None])* alpha2[:,:,None] * fg2)) / (alpha_new[:,:,None] + 1e-5))
    # The overlap of two 50% transparency should be 25%
    
    sample = {'fg': fg_new, 'alpha': alpha_new, 'bg': bg, 'image_name': 'm'}
    return sample
